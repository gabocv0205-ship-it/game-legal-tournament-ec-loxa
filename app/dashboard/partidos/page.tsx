...
export default function PartidosPage() {
  ...
  const [mostrarAuspiciantesPoster, setMostrarAuspiciantesPoster] = useState(false);
  ...
  const imprimirPlanilla = async (partido: any, esEstandar = false) => {
    ...
    const altoFilaMm = cupoPartido > 22 ? 3.8 : cupoPartido > 18 ? 4.5 : cupoPartido > 14 ? 5.4 : 6.2;
    ...
    const html = `<!DOCTYPE html><html lang="es"><head><title>Planilla oficial</title><style>
      @page{size:A4 landscape;margin:3mm}*{box-sizing:border-box}html,body{width:100%;height:100%;margin:0}body{font-family:Arial,Helvetica,sans-serif;color:#111827;background:#fff}.sheet{width:291mm;height:204mm;padding:2mm;border:2px solid #111827;display:grid;grid-template-columns:minmax(0,1fr) 0 minmax(0,1fr);gap:2mm;overflow:hidden}.cut-line{border-left:1.5px dashed #111827;height:100%}.team-half{min-width:0;height:100%;display:flex;flex-direction:column;border:1px solid #111827;padding:2mm;page-break-inside:avoid;break-inside:avoid}.half-top{display:flex;align-items:center;justify-content:space-between;border-bottom:2px solid #0f5132;padding-bottom:1mm}.half-top strong{font-size:10px;text-transform:uppercase;letter-spacing:.7px}.half-top span{font-size:8px;font-weight:900;color:#0f5132}.match-title{text-align:center;font-size:9px;font-weight:900;text-transform:uppercase;padding:1mm 0}.match-title b{color:#0f5132;margin:0 2mm}.match-meta{display:grid;grid-template-columns:.55fr 1fr 1.3fr;gap:1mm;font-size:6.4px;background:#f3f4f6;border:1px solid #cbd5e1;padding:1mm}.match-meta span{min-width:0;border-bottom:1px solid #9ca3af}.team-name-row{display:grid;grid-template-columns:auto auto 1fr;align-items:end;gap:1.2mm;margin-top:1mm;font-size:8px}.team-name-row span{border:1px solid #0f5132;background:#0f5132;color:#fff;padding:.8mm 1.4mm;font-weight:900;text-transform:uppercase}.team-name-row b{text-transform:uppercase}.team-name-row i{display:block;height:5mm;border-bottom:1.5px solid #111827}.team-rules{font-size:6.4px;text-align:center;text-transform:uppercase;font-weight:900;color:#475569;padding:.8mm 0}.players-table{width:100%;border-collapse:collapse;table-layout:fixed;font-size:5.6px}.players-table th,.players-table td{border:1px solid #64748b;padding:.35mm;text-align:center;height:${altoFilaMm}mm;line-height:1}.players-table th{background:#e5e7eb;color:#111827;text-transform:uppercase;font-weight:900}.players-table th:nth-child(1),.players-table td:nth-child(1){width:5mm}.players-table th:nth-child(2),.players-table td:nth-child(2){width:23mm}.players-table th:nth-child(4),.players-table td:nth-child(4){width:7mm}.players-table th:nth-child(5),.players-table td:nth-child(5),.players-table th:nth-child(6),.players-table td:nth-child(6),.players-table th:nth-child(7),.players-table td:nth-child(7),.players-table th:nth-child(8),.players-table td:nth-child(8){width:6.5mm}.players-table .idx{background:#f8fafc;font-weight:900;color:#0f5132}.suspended{border:1px solid #f1b0a7;border-left:3px solid #b42318;background:#fff5f5;padding:.8mm;font-size:5.7px;margin-top:1mm;min-height:5mm;page-break-inside:avoid;break-inside:avoid}.observ-mini{border:1px solid #111827;padding:1mm;font-size:5.6px;min-height:18mm;margin-top:1mm}.observ-mini b{display:block;text-transform:uppercase;margin-bottom:.3mm}.observ-mini ul{margin:0 0 .3mm 0;padding-left:3mm}.observ-mini p{margin:0}.lines span{display:block;height:4px;border-bottom:1px solid #cbd5e1}.team-footer{display:grid;grid-template-columns:1fr 1fr;gap:3mm;font-size:6px;margin-top:auto;padding-top:2mm}.team-footer div{border-top:1px solid #111827;min-height:8mm;padding-top:1mm;text-align:center}
      @media print{html,body{width:297mm;height:210mm}.sheet{width:291mm;height:204mm;margin:0;overflow:hidden;page-break-inside:avoid;break-inside:avoid}.team-half,.players-table{page-break-inside:avoid;break-inside:avoid}}
  ...
  const ubicacionPoster = useMemo(() => {
    const canchas = Array.from(new Set(partidosPoster.map(partido => String(partido.court || "").trim()).filter(Boolean)));
    if (canchas.length === 1) return canchas[0];
    if (canchas.length > 1) return `Sedes de la jornada: ${canchas.join(" · ")}`;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 border-b border-[#2E2E2E] pb-4 gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {auspiciantesTorneo.length > 0 && (
              <label className="flex items-center gap-2 text-[10px] text-gray-300 font-bold uppercase">
                <input type="checkbox" checked={mostrarAuspiciantesPoster} onChange={e => setMostrarAuspiciantesPoster(e.target.checked)} className="accent-[#D4A017]" />
      ...
      <div style={{ display: "none" }} ref={jornadaPosterRef}>
        <div
          className="poster-export-canvas relative w-[1080px] overflow-hidden bg-[#06183a] px-10 py-9 text-white"
          style={fondoPosterUrl && usarFondoPersonalizado ? { minHeight: `${alturaPosterJornada}px`, backgroundImage: `linear-gradient(135deg, rgba(2,12,34,.84), rgba(4,24,58,.78)), url("${fondoPosterUrl}")`, backgroundSize: "cover", backgroundPosition: "center", fontFamily: posterFontFamily } : { minHeight: `${alturaPosterJornada}px`, backgroundImage: "radial-gradient(circle at 10% 20%, rgba(212,160,23,.32), transparent 24%), radial-gradient(circle at 92% 70%, rgba(212,160,23,.24), transparent 22%), linear-gradient(145deg, #03102c, #063b78 52%, #020817)", fontFamily: posterFontFamily }}
        >
          ...
          <div className="relative z-10 flex items-start justify-between gap-6">
            ...
            <div className="rounded-xl border border-[#D4A017]/60 bg-black/45 p-2 text-center shadow-2xl">
              {appUrl && <QRCodeSVG value={`${appUrl}/torneo/${torneoSlug}`} size={96} level={"H"} fgColor="#D4A017" bgColor="#071735" />}
                  ...
                  {partidosPorDiaPoster[dia].map(p => (
                    <div key={p.id} className="grid grid-cols-[126px_minmax(0,1fr)_84px_minmax(0,1fr)] items-center overflow-hidden rounded-xl border border-white/90 bg-white px-4 py-3 text-[#072047] shadow-[0_10px_22px_rgba(0,0,0,.28)]">
                      ...
                      <div className="col-span-4 mt-2 flex items-center justify-between gap-3 border-t border-[#072047]/15 pt-1 text-[8px] font-black uppercase tracking-[0.15em] text-[#38506c]">
                        ...
                        <span className="max-w-[620px] break-words text-right leading-tight">{p.court || "Cancha por confirmar"}</span>
                      </div>
                    </div>
                  ))}
                </div>
