...
export type TeamInvitationSheetReport = {
  tournamentName: string;
  teamName: string;
  teamShieldUrl?: string;
  managerName: string;
  ...
  managerEmail?: string;
  generatedAt?: string;
  rows: Array<Pick<TeamPlayerPdfRow, "index" | "fullName" | "identification"> & { jerseyNumber?: string }>;
  blankSlots?: number;
};
...
export function exportTeamInvitationSheetPdf(report: TeamInvitationSheetReport, filename: string) {
  ...
  const rows = report.rows.length
    ? report.rows
    : Array.from({ length: Math.max(0, report.blankSlots || 0) }, (_, index) => ({ index: index + 1, identification: "", fullName: "", jerseyNumber: "" }));
  const rowsHtml = rows.length
    ? rows.map(row => `<tr><td class="number">${row.index}</td><td class="identification">${escapeHtml(row.identification)}</td><td class="player-name">${escapeHtml(row.fullName)}</td><td class="jersey">${escapeHtml(row.jerseyNumber || "")}</td></tr>`).join("")
    : `<tr><td colspan="4" class="empty">No existen jugadores registrados para este equipo en el torneo seleccionado.</td></tr>`;
  const generatedAt = report.generatedAt ? `<p>Generada: ${escapeHtml(report.generatedAt)}</p>` : "";
  const html = `<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Ficha de invitacion - ${escapeHtml(report.teamName)}</title><style>
    @page { size: A4 portrait; margin: 10mm; } * { box-sizing: border-box; } body { margin: 0; background: #f4f6f8; color: #152235; font-family: "Arial Nova Light", Inter, "Segoe UI", Arial, sans-serif; } .page { min-height: 100vh; border: 1px solid #d7dee8; background: #fff; padding: 14px; } .header { display: flex; align-items: center; justify-content: space-between; gap: 16px; border-radius: 14px; padding: 14px; background: #101827; border-bottom: 5px solid #d4a017; color: #fff; } h1 { margin: 0; font-size: 20px; letter-spacing: .09em; text-transform: uppercase; } h2 { margin: 6px 0 0; color: #f5c842; font-size: 14px; letter-spacing: .12em; text-transform: uppercase; } .header p { margin: 5px 0 0; color: #d9e0e9; font-size: 10px; } .shield { width: 70px; height: 70px; flex: 0 0 70px; border: 1px solid #d4a017; border-radius: 12px; padding: 6px; background: #fff; object-fit: contain; } .placeholder { display: grid; place-items: center; color: #101827; font-size: 20px; font-weight: 900; } .title { margin: 14px 0 7px; color: #172033; font-size: 13px; font-weight: 900; letter-spacing: .12em; text-transform: uppercase; } .manager { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 12px; } .card { min-width: 0; border: 1px solid #9eabbc; border-radius: 8px; background: #fff; padding: 10px 12px; } .label { margin: 0 0 4px; color: #596678; font-size: 8px; font-weight: 900; letter-spacing: .14em; text-transform: uppercase; } .value, .card p { margin: 0; color: #172033; font-size: 11px; line-height: 1.55; } .value { font-size: 13px; font-weight: 800; } table { width: 100%; border-collapse: collapse; table-layout: fixed; background: #fff; font-size: 9px; } th { border: 1px solid #27364a; padding: 7px 5px; background: #101827; color: #f5c842; font-size: 7px; letter-spacing: .08em; text-align: center; text-transform: uppercase; } td { height: 24px; border: 1px solid #66758a; padding: 4px 6px; color: #172033; } .number { width: 6%; text-align: center; font-weight: 800; } .identification { width: 28%; } .player-name { width: 54%; } .jersey { width: 12%; text-align: center; } .empty { padding: 20px; text-align: center; color: #6b7788; font-style: italic; } .footer { margin-top: 12px; border-top: 1px solid #dce3eb; padding-top: 8px; color: #6b7788; font-size: 8px; letter-spacing: .1em; text-align: center; text-transform: uppercase; } @media print { body { background: #fff; } .page { min-height: auto; border: 0; padding: 0; } .header, .card, tr { break-inside: avoid; page-break-inside: avoid; } }
  </style></head><body><main class="page"><header class="header"><div><h1>${escapeHtml(report.tournamentName)}</h1><h2>Ficha de invitacion - ${escapeHtml(report.teamName)}</h2>${generatedAt}</div>${shield}</header><h3 class="title">Datos del equipo y dirigente</h3><section class="manager"><div class="card"><p class="label">Equipo invitado</p><p class="value">${escapeHtml(report.teamName)}</p></div><div class="card"><p class="label">Dirigente responsable</p><p class="value">${escapeHtml(report.managerName || "Sin registrar")}</p><p><strong>Contacto:</strong> ${escapeHtml(report.managerPhone || "Sin registrar")}</p>${managerEmail}</div></section><h3 class="title">Nomina de jugadores</h3><table><colgroup><col style="width:6%"><col style="width:28%"><col style="width:54%"><col style="width:12%"></colgroup><thead><tr><th>#</th><th>Cedula / identificacion</th><th>Nombres completos</th><th>Dorsal</th></tr></thead><tbody>${rowsHtml}</tbody></table><footer class="footer">Game Legal Tournament · Ficha de invitacion por equipo</footer></main></body></html>`;
  const popup = window.open("", "_blank");
  if (!popup) { downloadBlob(new Blob([html], { type: "text/html;charset=utf-8" }), filename.replace(/\.pdf$/i, ".html")); return; }
  popup.document.write(html);
  popup.document.close();
  popup.focus();
  setTimeout(() => popup.print(), 250);
